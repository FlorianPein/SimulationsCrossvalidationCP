...
for (i in 1:M) {
  ...
  Y[indices] <- Y[indices] + rpois(10, 30)
...
saveRDS(est, file = paste("results/EstBlocks2028out30.RDS", sep = ""))
saveRDS(mse, file = paste("results/MseBlocks2028out30.RDS", sep = ""))
...
tab <- xtable(tab, digits = rep(4, 6), auto = TRUE, display = c("g","s","g","g","g", "g"),
              align = c("l", "l|", "c", "c", "c", "c"),
              caption = "test")
saveRDS(tab, file = paste("results/Blocks2028out30.RDS", sep = ""))
print(tab, include.rownames = FALSE, file = paste("results/Blocks2028out30.tex", sep = ""),
