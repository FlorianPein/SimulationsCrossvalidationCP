...
signal <- c(rep(delta, n / 2 - lambda), rep(0, lambda), rep(Delta, n / 2))
name <- paste0("under_D_", D)
...
R.matlab::writeMat(paste0("matData/", name, "/DataRev.mat"), y = Yrev)


#####
# run runRev.m in Matlab before continuing
#####

namesMethod <- c("arlotV2", "arlotV5", "arlotV2rev", "arlotV5rev")
abbr <- c("V2", "V5", "V2rev", "V5rev")
est <- list()
mse <- list()
...
est$arlotV2rev <- as.numeric(R.matlab::readMat(paste0("matData/", name, "/numberV2rev.mat"))$numberjumpsV2rev)
est$arlotV5rev <- as.numeric(R.matlab::readMat(paste0("matData/", name, "/numberV5rev.mat"))$numberjumpsV5rev)
