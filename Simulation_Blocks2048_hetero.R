...
for (i in 1:M) {
  ...
  sigma <- numeric(n)
  for (k in 1:(length(cp) - 1))
    sigma[(cp[k] + 1):(cp[k + 1])] <- runif(1, 3, 11)
...
saveRDS(est, file = paste("results/EstBlocks2028hetero.RDS", sep = ""))
saveRDS(mse, file = paste("results/MseBlocks2028hetero.RDS", sep = ""))
...
tab <- xtable(tab, digits = rep(4, 6), auto = TRUE, display = c("g","s","g","g","g", "g"),
              align = c("l", "l|", "c", "c", "c", "c"),
              caption = "test")
saveRDS(tab, file = paste("results/Blocks2028hetero.RDS", sep = ""))
print(tab, include.rownames = FALSE, file = paste("results/Blocks2028hetero.tex", sep = ""),
