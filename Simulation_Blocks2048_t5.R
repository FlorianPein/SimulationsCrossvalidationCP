...
for (i in 1:M) {
  ...
  set.seed(seed + i)
  df <- 5
  Y <- rt(n, df) / sqrt(df / (df - 2)) * sigma + signal
...
saveRDS(est, file = paste("results/EstBlocks2028t5.RDS", sep = ""))
saveRDS(mse, file = paste("results/MseBlocks2028t5.RDS", sep = ""))
...
tab <- xtable(tab, digits = rep(4, 6), auto = TRUE, display = c("g","s","g","g","g", "g"),
              align = c("l", "l|", "c", "c", "c", "c"),
              caption = "test")
saveRDS(tab, file = paste("results/Blocks2028t5.RDS", sep = ""))
print(tab, include.rownames = FALSE, file = paste("results/Blocks2028t5.tex", sep = ""),
