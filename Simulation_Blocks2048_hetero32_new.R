...
for (i in 1:M) {
  ...
  sigma <- numeric(n)
  for (k in 1:(n / 32))
    sigma[((k - 1) * 32 + 1):(k * 32)] <- runif(1, 0, 8)
...
saveRDS(est, file = paste("results/EstBlocks2028hetero32_0to8.RDS", sep = ""))
saveRDS(mse, file = paste("results/MseBlocks2028hetero32_0to8.RDS", sep = ""))
...
tab <- xtable(tab, digits = rep(4, 6), auto = TRUE, display = c("g","s","g","g","g", "g"),
              align = c("l", "l|", "c", "c", "c", "c"),
              caption = "test")
saveRDS(tab, file = paste("results/Blocks2028hetero32_0to8.RDS", sep = ""))
print(tab, include.rownames = FALSE, file = paste("results/Blocks2028hetero32_0to8.tex", sep = ""),
